...
public class CamSaveData extends SavedData {
    ...
    public static final String DATA_NAME = CinematicTools.MODID + "_Scenes";

    private HashMap<String, CamScene> scenes = new HashMap<>();

    public CamSaveData() {}

    public static CamSaveData load(CompoundTag nbt, HolderLookup.Provider provider) {
        CamSaveData data = new CamSaveData();
        for (String key : nbt.getAllKeys())
            try {
                data.scenes.put(key, new CamScene(nbt.getCompound(key)));
